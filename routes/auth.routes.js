...
router.post('/signup', (req, res, next) => {
  ...
  router.put('/profile/:userId/edit', async (req, res, next) => {
    ...
    if (email === '' || name === '' || !username) {
      res.status(400).json({ message: "Provide email, name and username" });
      return;
    }
   ...
   try {
        ...
        if (existingUser) {
          ...
          const updatedFields = { email, name, username };
          ...
          const { _id, email: updatedEmail, name: updatedName, username: updatedUsername } = updatedUser;
          res.status(200).json({ user: { _id, email: updatedEmail, name: updatedName, username: updatedUsername } });
   } catch (error) {
        console.log(err)
   }
  });
